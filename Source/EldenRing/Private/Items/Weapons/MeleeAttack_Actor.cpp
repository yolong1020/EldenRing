...
void AMeleeAttack_Actor::OnBoxOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if (nullptr != component)
	{
		...
		if (nullptr != defender && component == defender->GetParrySphere())
		{
			AGameCharacter* attacker = Cast<AGameCharacter>(owner);
			if (nullptr != attacker) { attacker->OnTakeStunn(); }
		}
	}
	...
	if (nullptr != shield)
	{
		...
		if (attacker->IsCurrentActionState(FString("Attacking")) && defender->IsGuardState())
		{
	...
	else
	{
		...
		if (owner->ActorHasTag("NPC") && is_character) { Cast<ANPC_Character>(owner)->SuccessAttack(); }
	}
}
